...
from services import (
    ...
    add_product_to_cart, get_cart, checkout_cart,
        ...
        with SessionLocal() as db:
            ...
            for cat in categories:
                ...
                for prod in products:
                    ...
                    markup.add(types.InlineKeyboardButton(f"➕ {prod.name}", callback_data=f"add_{prod.id}"))
...
class CartHandler:
    ...
    def show_cart(self, message):
        ...
        with SessionLocal() as db:
            ...
            if not cart or not cart.content.get('products'):
                self.bot.send_message(message.chat.id, "Корзина пуста.", reply_markup=self.main_menu)
                return
    ...
    def clear_cart(self, call):
        ...
        self.bot.send_message(call.message.chat.id, "Корзина очищена.", reply_markup=self.main_menu)

    def ask_payment_method(self, call, order_id):
        """Показывает выбор способа оплаты с номером заказа."""
        markup = types.InlineKeyboardMarkup()
        markup.add(
            types.InlineKeyboardButton("💳 Онлайн", callback_data=f"pay_online_{order_id}"),
            types.InlineKeyboardButton("💵 Наличными", callback_data=f"pay_cash_{order_id}")
        )
        self.bot.send_message(call.message.chat.id, f"Выберите способ оплаты для заказа №{order_id}:", reply_markup=markup)
    ...
    def checkout(self, call):
        ...
        if order:
            self.bot.send_message(call.message.chat.id, f"✅ Заказ №{order.id} оформлен!", reply_markup=self.main_menu)
    ...
    def add_to_cart(self, call):
        ...
        self.bot.send_message(call.message.chat.id, "Добавлено. Продолжайте выбор или откройте 🛒 Корзину.")
    ...
    def show_orders(self, message):
        ...
        with SessionLocal() as db:
            ...
            for order in orders:
                ...
                self.bot.send_message(message.chat.id, text, parse_mode='HTML', reply_markup=self.main_menu)
...
class FeedbackHandler:
    ...
    def handle_feedback(self, message):
        ...
        self.bot.send_message(message.chat.id, "Напишите ваш отзыв. Он будет отправлен администратору.")
    ...
    def handle_review(self, message):
        ...
        try:
            ...
            self.bot.send_message(message.chat.id, f"Напишите отзыв для заказа №{order_id}:")
        except:
